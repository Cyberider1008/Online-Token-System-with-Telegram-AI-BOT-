    ...
    public partial class call : System.Web.UI.Page
    {
            ...
            try
            {
                if (DropDownList1.SelectedItem.Value != "Please Select Operator" && DropDownList2.SelectedItem.Value != "Please Select Department" && DropDownList3.SelectedItem.Value != "Please Select Counter")
                {
                    ...
                    else
                    {
                        ...
                        Response.Write("<script>alert(' not ant token issue!')</script>");
        ...
        private void SearchCustomers()
        {
            try
            {
                ...
                string sql = "select * from token1 where date=@date and department=@department and  not(token_no in(select token_no from missed union select token_no from call1) and department in(select department from missed union select department from call1) and mobile_no in(select mobile_no from missed union select mobile_no from call1) )";
                cmd.Parameters.AddWithValue("@date", date);
            ...
            try
            {
                if (DropDownList1.SelectedItem.Value != "Please Select Operator" && DropDownList2.SelectedItem.Value != "Please Select Department" && DropDownList3.SelectedItem.Value != "Please Select Counter")
                {
                    if (!string.IsNullOrEmpty(TextBox2.Text) && !string.IsNullOrEmpty(Label1.Text))
                    {
                        ...
                        string quary = "insert into missed values(@department,@token_no,@counter,@date,@operator,@name,@mobile_no)";
                        con.Open();
                        cmd = new SqlCommand(quary, con);
                        cmd.Parameters.AddWithValue("@department", DropDownList2.SelectedItem.Text);
                        cmd.Parameters.AddWithValue("@token_no", TextBox2.Text);
        ...
        public void callcustomer()
        {
            try
            {
                string sql = "select token_no,name,mobile_no from token1 where date=@date and department=@department and  not(token_no  in(select token_no from missed union select token_no from call1) and department in(select department from missed union select department from call1) and mobile_no in(select mobile_no from missed union select mobile_no from call1) )";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    hiddentxt.Value = dt.Rows[0][2].ToString();
                }
                else
                {
                    TextBox2.Text = "";
