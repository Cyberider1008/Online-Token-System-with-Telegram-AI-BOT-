...
{
    public partial class tokenapply : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Label2.Text = "+91" + "    " + TextBox2.Text;

                LoginDetails log = ValidateUser(TextBox2.Text);
                if (log.IsAuthUser)
                {
                ...
                {
                    Response.Write("<script>alert('Please Register account!')</script>");
        ...
        private LoginDetails ValidateUser(string username)
        {
            ...
            try
            {
                ...
                string query = "select * from usrdata1 where Mobile_No ='" + TextBox2.Text + "'";
                date = new SqlDataAdapter(query, con);
                con.Open();
                date.Fill(ds);
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            try
            {
                if (DropDownList2.SelectedItem.Value != "Please Select Department")
                {
                    ...
                    {
                        SqlCommand cmd = new SqlCommand("insert into token1 values('" + vd1.Text + "','" + Label2.Text + "','" + DropDownList1.SelectedItem.Value + "','" + Label1.Text + "','" + date + "','" + time + "','" + DropDownList2.SelectedItem.Value + "')", con);
        ...
        public void BindDropDown(string DateTime, string name)
        {
            try
            {
                if (DropDownList2.SelectedItem.Value != "Please Select Department")
                {
                    SqlCommand cmd = new SqlCommand("SELECT Top 1 limit FROM department where name = @Name", con);
                    con.Open();
